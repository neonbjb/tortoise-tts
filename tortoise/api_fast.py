...
class TextToSpeech:
    ...
    def handle_chunks(self, wav_gen, wav_gen_prev, wav_overlap, overlap_len):
        ...
        if wav_overlap is not None:
            ...
            crossfade_wav = crossfade_wav * torch.linspace(0.0, 1.0, overlap_len).to(
                crossfade_wav.device
            )
    ...
    def tts_stream(
        ...
        **hf_generate_kwargs,
        ...
        if voice_samples is not None:
            auto_conditioning = self.get_conditioning_latents(
                voice_samples, return_mels=False
            )
        else:
        ...
        with torch.no_grad():
            ...
            with torch.autocast(
                device_type="cuda" if not torch.backends.mps.is_available() else "mps",
                dtype=torch.float16,
                enabled=self.half,
            ):
            ...
            while not is_end:
                ...
                if is_end or (
                    stream_chunk_size > 0
                    and len(codes_) >= max(stream_chunk_size, first_buffer)
                ):
                    first_buffer = 0
        ...
        with torch.no_grad():
            ...
            with torch.autocast(
                ...
                gpt_latents = self.autoregressive(
                    ...
                    codes,
                    torch.tensor(
                        [codes.shape[-1] * self.autoregressive.mel_length_compression],
