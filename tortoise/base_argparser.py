...
ap.add_argument(
    "--kv_cache",
    help="no-op; kv_cache is enabled by default and this flag exists for compatibility",
    default=True,
    action="store_true",
)
ap.add_argument(
    "--no_cache",
    help="disable kv_cache usage. This should really only be used if you are very low on vram.",
    action="store_false",
    dest="kv_cache",
)
ap.add_argument(
...
ap.add_argument(
    "--cond_free", type=bool, help="enable/disable conditioning free diffusion."
)
ap.add_argument(
    "--cvvp_amount",
    type=float,
...
ap.add_argument(
    ...
    type=int,
    help="override the autoregressive_samples used for diffusion (default depends on --preset)",
)
ap.add_argument(
    "--original_tortoise",
    help="ensure results are identical to original tortoise-tts repo",
    default=False,
    action="store_true",
)
ap.add_argument(
    "--latent_averaging_mode", type=int, help="latent averaging mode (0/1/2)", default=0
)
ap.add_argument(
    "--ar-checkpoint",
    type=str,
